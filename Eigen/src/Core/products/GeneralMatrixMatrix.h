  ...
  GemmParallelInfo<Index>* info = 0)
{
  ...
  if(info)
  {
    ...
    for(Index k=0; k<depth; k+=kc)
    {
      ...
      // In order to reduce the chance that a thread has to sleep for the other,
      ...
      for(int shift=0; shift<threads; ++shift)
      {
        ...
        // However, no need to sleep for the B' part which has been updated by the current thread!
        if (shift>0) {
          while(info[i].sync!=k) {
          }
        }
